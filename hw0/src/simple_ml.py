...
def parse_mnist(image_filename, label_filename):
    ...
    with gzip.open(label_filename, "rb") as label:
        ...
        y = np.vstack([np.frombuffer(label.read(1), dtype=np.uint8) for i in range(n)]).flatten()
    with gzip.open(image_filename, "rb") as image:
        magic, num, rows, cols = struct.unpack('>IIII', image.read(16))
        print(num)
        X = np.vstack([np.frombuffer(image.read(rows * cols), dtype=np.uint8) for i in range(num)])
...
def softmax_loss(Z, y):
    ...
    # loss = -np.log(Z[np.arange(Z.shape[0]), y]) + np.log(np.sum(Z, axis=1))
    # return np.mean(loss)
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(iterations):
        ...
        Y = np.zeros((batch, y.max()+1))
        Y[np.arange(batch), y_] = 1 # one-hot ground truth

        G2 = Z - Y
        G1 = np.zeros_like(x @ W1)
        G1[x @ W1 > 0] = 1
        G3 = G1
        G1 = G3 * (G2 @ W2.T)
        # grad1 = x.T @ G1 / batch
        ...
        # W1 -= lr * grad1
        # W2 -= lr * grad2
        grad2 = np.maximum(x @ W1, 0).T @ (Z - Y) / batch # z1.T @ G2 / batch
        assert(grad2.shape == W2.shape)
        grad1 = x.T @ ((Z - Y) @ W2.T * (x @ W1 > 0))/ batch 
        assert(grad1.shape == W1.shape)
        
        W2 -= lr * grad2
        W1 -= lr * grad1
        
        # grad = x.T @ ((Z - Y) @ W2.T * G3) / batch # Err
...
def train_softmax(X_tr, y_tr, X_te, y_te, epochs=100, lr=0.5, batch=100,
                  cpp=False):
